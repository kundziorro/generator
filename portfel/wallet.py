...
class Wallet:
    ...
    def show_transactions(self) -> None:
        for transaction in self.transactions:
            ...
            print(
                f"In {transaction.date} {sign}{transaction.value} EUR by rate {transaction.rate}"
            )

    def add_buy_transaction(self, value_euro, date, rate) -> None:
        self.exchange_rate_requester.get_todays_rate()
        buy = Transaction(date, value_euro, rate)
        self.transactions.append(buy)

    def add_sell_transaction(self, value_euro, date, rate) -> None:
